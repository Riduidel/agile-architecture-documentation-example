...
Softare system will be deployed on Kubernetes using Jenkins-X (itself already installed as operator on Kubernetes).
As this example is not live, there is no real deployment that would allow auto-discovery.
As a consequence, deployment diagram will be only "virtual" (in the sense that it is simply written).

[plantuml, {plantumldir}deployment, svg, opts="interactive"]     
----
include::{structurizrdir}/deployment.plantuml[]
----
////
